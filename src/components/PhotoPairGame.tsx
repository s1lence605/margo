...
export default function PhotoPairGame({ handleShowProposal }: ValentinesProposalProps) {
  ...
  // Generate shuffled deck on client only (fix hydration mismatch)
  useEffect(() => {
    setDeck(shuffleArray(imagePairs));
  }, []);

  const deckReady = deck.length === imagePairs.length; // true when 36 cards ready
  ...
  useEffect(() => {
    if (deckReady && matched.length === imagePairs.length) {
      handleShowProposal();
    }
  }, [matched, deckReady, handleShowProposal]);
        ...
        index !== null ? (
          <motion.div
            ...
            className={`w-[11vh] h-[11vh] lg:w-20 lg:h-20 relative ${
              deckReady ? "cursor-pointer" : "cursor-default"
            }`}
            whileHover={deckReady ? { scale: 1.1 } : undefined}
          ...
          >
            ...
            {/* Front of the card */}
            {(selected.includes(index) || matched.includes(index)) && deckReady && (
              <motion.div
